<template>
  <AdminLayout page-title="Edit Post">
    <div class="row">
      <div class="col-lg-8">
          ...
          <div class="card-body">
            ...
            <div class="d-flex align-items-start mb-3">
              ...
              <div>
                ...
                <p class="text-muted mb-0 small">{{ form.author_name || '-' }}</p>
              </div>
            </div>
...
async function fetchPost() {
  ...
  try {
    ...
    form.image_url = post.foto || post.image_url || post.image || ''
    form.created_at = post.created_at || post.tanggal_post || ''
    form.tanggal_post = post.tanggal_post || post.created_at || ''
    form.author_name = post.author_name || post.author?.name || '-'
    imagePreview.value = ''
    selectedFile.value = null
  } catch (error) {
...
async function handleSubmit() {
  ...
  // Penting: tambahkan _method agar Laravel mengenali sebagai update
  ...
  try {
    ...
    if (error.response?.status === 422) {
      ...
      alert('Gagal update post. Pastikan data sudah benar.')
    }
  } finally {
    loading.value = false
  }
